...
phys <- rdist( unique( cbind( baja$Long, baja$Lat ) ))
...
\begin{enumerate}
	\item Using the 2-D geometry in Figure \ref{fig:amova_geometry}, what is the distance between to heterozygotes that do not share any alleles (e.g., a 4-allele locus)?
	\item To what extent are Cavalli-Sforza, Nei, and cGD correlated?
\end{enumerate}
