...
%\VignetteIndexEntry{Parent Offspring Data}
...
<<echo=true,eval=true>>=
require(gstudio)
baseURL <- "https://docs.google.com/spreadsheet/pub?hl=en_US&hl=en_US&"
key <- "key=0Aq-lsUWPDuZtdF9xMXZGQWNtbk1FNTVWd3F3U0FDdXc&single=true&gid=0&"
range <- "range=A1%3AI63&output=csv"
cornusURL <- paste(baseURL,key,range,sep="")
cornus <- read.population( googleURL=cornusURL)
@
...
\begin{figure}
	\centering
	\includegraphics{ParentOffspringData-spiderplot}
